...
def compute_arrival_points(data, lat, lon, start_datetime, end_datetime):
    ...
    for num_changement in range(4):
        ...
        reached_stops = pd.concat(
            [
                reached_stops,
                reachable_stoptimes.loc[:, ["stop_id", "arrival_datetime"]],
            ]
        ).sort_values(by="arrival_datetime").drop_duplicates(subset=["stop_id"], keep="first")
    ...
    reached_stops = pd.concat(
                            [reached_stops, reachable_stoptimes.loc[:, ["stop_id", "arrival_datetime"]]]
        ).sort_values(by="arrival_datetime").drop_duplicates(subset=["stop_id"], keep="first")
